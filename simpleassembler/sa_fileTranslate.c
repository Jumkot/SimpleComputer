...
sa_fileTranslate (char *input_file, char *output_file)
{
    ...
    if (!file_sa)
    {
      error_length = snprintf (error_string, 100, "Ошибка: Файл '%s' не существует.\n",
                             input_file);
    ...
    for (int i = 0; !feof (file_sa); i++)
    {
      ...
      if (!fgets (string, 160, file_sa))
        {
          if (feof (file_sa))
            ...
            {
              error_length = snprintf (error_string, 100, "Ошибка: невозможно прочитать строку %d\n", i);
                write (1, error_string, error_length);
                return -1;
            }
        }
      ...
      if (address < 0 || address > 127)
        {
            error_length = snprintf (error_string, 100, "Ошибка: неверный адрес ячейки '%d' в строке %d\n", address, i);
            write (1, error_string, error_length);
            return -1;
        }
      ...
      if (ptr != NULL && ptr[0] != ';')
        {
            error_length = snprintf (error_string, 100, "Ошибка: неверный знак '%c' в строке %d\n", ptr[0], i);
            write (1, error_string, error_length);
            return -1;
        }
      ...
      if (sc_commandEncode (0, command, operand, &value))
        {
            error_length = snprintf (error_string, 100, "Ошибка кодирования в строке %d\n", i);
            write (1, error_string, error_length);
            return -1;
        }
